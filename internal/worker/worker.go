...
import (
	...
	"github.com/lacsar712/corebore/internal/backoff"
	"github.com/lacsar712/corebore/internal/classify"
	"github.com/lacsar712/corebore/internal/clock"
	"github.com/lacsar712/corebore/internal/lab"
	"github.com/lacsar712/corebore/internal/destination"
	"github.com/lacsar712/corebore/internal/dlq"
	"github.com/lacsar712/corebore/internal/job"
	"github.com/lacsar712/corebore/internal/journal"
	"github.com/lacsar712/corebore/internal/queue"
	"github.com/lacsar712/corebore/internal/runtime"
)
...
func New(clk clock.Clock, broker *queue.Broker, dests *destination.Registry, gates *runtime.Gates, client *lab.Client, jlog *journal.Log, dead *dlq.Queue, policy backoff.Policy) *Engine {
	return &Engine{
		clk:    clk,
		broker: broker,
		dests:  dests,
...
func (e *Engine) tick(ctx context.Context) {
	...
	defer e.broker.Release(j.DestinationID)
	e.handle(ctx, j)
}

func (e *Engine) handle(ctx context.Context, j job.Job) {
	dest, ok := e.dests.Get(j.DestinationID)
	if !ok || !dest.Enabled {
		e.dead.Push(dlq.FromJob(j, "destination missing or disabled", e.clk.Now()))
		e.note(j, "terminal", 0, "", "destination_unavailable")
		return
	}
	...
	if wait > 0 {
		...
		e.broker.Enqueue(j, dest.Ordered, dest.MaxInFlight)
		e.note(j, "skipped_rate", 0, "", "rate_limited")
		return
	}

	j.Attempt++
	res := e.client.Post(ctx, lab.Request{
		URL:           dest.URL,
		Secret:        dest.Secret,
		EventID:       j.EventID,
