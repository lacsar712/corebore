...
import (
	...
	"github.com/lacsar712/corebore/internal/event"
...
func (in CreateInput) normalize() (CreateInput, error) {
	...
	for _, p := range in.TypePrefixes {
		...
		if err := event.ValidateType(strings.TrimSuffix(p, ".")); err != nil && !strings.HasSuffix(p, ".") {
			// prefix "order." is allowed; validate the stem
...
func (d Destination) Matches(eventType string) bool {
	if !d.Enabled {
		return false
	}
...
func (d Destination) OutboundSecrets(now time.Time) []string {
...
func NewRegistry(clk clock.Clock) *Registry {
	return &Registry{clk: clk, items: make(map[string]Destination)}
}

func (r *Registry) Create(in CreateInput) (Destination, error) {
	in, err := in.normalize()
	if err != nil {
...
func (r *Registry) Put(d Destination) error {
	if d.ID == "" {
...
func (r *Registry) SetEnabled(id string, enabled bool) (Destination, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	d, ok := r.items[id]
	if !ok {
		return Destination{}, fmt.Errorf("destination %s not found", id)
	}
	d.Enabled = enabled
	d.UpdatedAt = r.clk.Now()
	r.items[id] = d
...
func (r *Registry) RotateSecret(id, next string, overlap time.Duration) (Destination, error) {
	if len(next) < 8 {
		return Destination{}, fmt.Errorf("secret must be at least 8 characters")
	}
	r.mu.Lock()
	defer r.mu.Unlock()
	d, ok := r.items[id]
...
func (r *Registry) List() []Destination {
	r.mu.Lock()
	defer r.mu.Unlock()
	out := make([]Destination, 0, len(r.items))
...
func (r *Registry) Matching(eventType string) []Destination {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *Registry) Restore(items []Destination) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.items = make(map[string]Destination, len(items))
	for _, d := range items {
		r.items[d.ID] = d
	}
}

func (r *Registry) Public(d Destination) map[string]any {
