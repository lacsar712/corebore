...
async function refreshDests() {
  const data = await api("/api/v1/destinations");
  $("destinations").innerHTML = (data.destinations || []).map((d) => `
    <div class="row">
      <div>
...
async function refreshAll() {
  await refreshMeta();
  await Promise.all([refreshDests(), refreshJournal(), refreshDlq(), refreshLoop()]);
}
...
$("event-form").addEventListener("submit", async (ev) => {
  ...
  const sig = "v1=" + await hmacHex("dev-ingest-secret", canonical);
  try {
    const res = await api("/api/v1/events", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-Core-Timestamp": String(ts),
        "X-Core-Nonce": n,
        "X-Core-Signature": sig,
        "Idempotency-Key": idemKey(),
        "X-Core-Source-Key": "ingest"
      },
      body
    });
...
document.body.addEventListener("click", async (ev) => {
  ...
  if (t.dataset.toggle) {
    ...
    await api(`/api/v1/destinations/${t.dataset.toggle}/enable`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
