...
import (
	...
	"github.com/lacsar712/corebore/internal/circuit"
...
func newTestEngineOpts(t *testing.T, url string, clk clock.Clock, opts engineOpts) (*Engine, *destination.Destination) {
	t.Helper()
	if opts.rate <= 0 {
		opts.rate = 100
	}
	...
	gates := runtime.NewGatesWithCircuit(clk, opts.circuit)
	e := New(clk, broker, dests, gates, lab.New(opts.timeout), journal.New(50), dlq.New(50), backoff.Policy{
...
func TestHandleSuccessClearsBreaker(t *testing.T) {
	...
	snap := e.gates.Breaker(dest.ID).Snapshot()
